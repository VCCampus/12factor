...
### Project Architecture Decision (2025-08-28, Updated 2025-09-09)

#### Core Decisions (Updated for uni-app Migration)
1. **uni-app as primary framework** - Migration from Vue to uni-app for cross-platform compatibility
2. **Complete architecture refactor** - Direct replacement of existing Vue architecture 
3. **Multi-platform build strategy** - Support for H5/Web and WeChat Mini Program

#### Build Output Strategy (Updated)
- **uni-app H5 build**: Builds to `/dist` directory (maintains web compatibility)
- **WeChat Mini Program**: Builds to `/dist-wxmp` directory  
- **Build script**: `./scripts/build-uni.sh` handles uni-app multi-platform build pipeline
- **Legacy Vue project**: Archived, no longer in active development

#### uni-app Migration Architecture
- **Framework**: Vue 3 + uni-app for cross-platform development
- **State Management**: Pinia (maintained for consistency)
- **Styling**: Tailwind CSS + uView + Neobrutalism design system
- **TypeScript**: Maintained with uni-app specific type definitions
- **Cross-platform Components**: UniView/UniText compatibility layer for Web fallback
...
#### 3. Directory Conventions (Updated for uni-app)
- `/uni-app` - **Primary source code entry point** (all new development in uni-app framework)
- `/dist` - **H5 build output** (website deployment, auto-generated by build-uni.sh)
- `/dist-wxmp` - **WeChat Mini Program build output**
- `/docs` - Documentation (managed)
- `/scripts` - Utility scripts (managed)
- `/logs` - Development and test logs
- `/vue` - Legacy Vue source (archived, reference only)
